...
const ProductDetailPage = ({ params }: ProductDetailPageProps) => {
  ...
  const imageUrl = product.imageUrl || "/default-image.jpg";
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full md:w-96 h-auto relative">
            <Image
              src={imageUrl}
              alt={product.name}
              className="object-cover rounded-lg w-full md:w-96 h-auto"
              width={400}
              height={400}
            />
          </div>
          ...
          {/* Product Details */}
          <div className="flex-grow">
            <h1 className="text-2xl md:text-4xl font-semibold text-gray-900">{product.name}</h1>
            <p className="mt-2 text-gray-600">{product.description}</p>

            {/* Product Price */}
            <div className="mt-4 text-lg md:text-xl font-semibold text-gray-900">
              ${product.price}
              {product.discountPercentage && (
                <span className="ml-2 text-red-500 line-through">
                  ${((product.price * (1 - product.discountPercentage / 100))).toFixed(2)}
            ...
            <div className="mt-4">
              <button
                onClick={handleAddToCart}
                className="w-full md:w-auto px-6 py-2 bg-[#fb2e86] text-white rounded-lg hover:bg-[#f8066f]"
              >
                Add to Cart
              </button>
            </div>
